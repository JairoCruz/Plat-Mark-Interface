import {
  ...
  import usePlatMark from "../../../src/hooks/usePlatMark";
  ...
  const Home = () => {
    ...
    const getPlatMarkData = useCallback(async () => {
        if (platMark) {
            const totalSupply = await platMark.methods.totalSupply().call();
            const dnaPreview = await platMark.methods.deterministicPseudoRandomDNA(totalSupply, account).call();
            const image = await platMark.methods.imageByDNA(dnaPreview).call();
            setImageSrc(image);
